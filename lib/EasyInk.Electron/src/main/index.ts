...
function createWindow(context: AppContext): void {
  const mainWindow = new BrowserWindow({
    width: 1180,
    height: 760,
    minWidth: 980,
    minHeight: 640,
    show: !context.config.startMinimized,
  ...
  mainWindow.on('ready-to-show', () => {
    if (!context.config.startMinimized) {
      mainWindow.show()
    }
  })
...
app.whenReady().then(async () => {
  electronApp.setAppUserModelId('com.easyink.electron')
  ...
  app.on('activate', function () {
    ...
    if (BrowserWindow.getAllWindows().length === 0 && appContext) createWindow(appContext)
  })
})
