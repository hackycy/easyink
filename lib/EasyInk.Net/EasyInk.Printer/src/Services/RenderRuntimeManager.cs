...
internal sealed class RenderRuntimeManager : IDisposable
{
    ...
    public string InstallBrowserVersion(string versionKey, IProgress<RenderBrowserInstallProgress>? progress, CancellationToken cancellationToken = default)
    {
        ...
        var option = RenderBrowserVersionCatalog.GetOption(normalizedKey);
        ...
        var executableCandidates = ResolveExecutableCandidates(manifest, option);
        ...
        if (download == null)
        {
            var systemBrowser = LocateSystemBrowser();
            ...
            throw CreateBrowserMissingException(normalizedKey, browserDir);
        }
    ...
    private static string[] ResolveExecutableCandidates(ManifestBrowserInfo? manifest, RenderBrowserVersionOption option)
    {
        var candidates = new List<string?> { "chrome.exe", "chromium.exe" };
    ...
    internal static string ExtractBrowserArchive(string archivePath, ManifestBrowserInfo? manifest, IEnumerable<string?> executableHints, string targetRoot)
    {
        if (!File.Exists(archivePath))
            throw new FileNotFoundException("Render Chromium zip 包不存在", archivePath);
        if (!string.Equals(Path.GetExtension(archivePath), ".zip", StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException("Render Chromium 包仅支持 .zip 文件");
        ...
        if (browserPath == null)
        {
            ...
            throw new FileNotFoundException(
                "Render Chromium zip 包中未找到可执行文件。候选: " + string.Join(", ", executableCandidates) + "; zip entries: " + entries,
                targetDir);
        }
    ...
    private BrowserDownloadInfo? ResolveBrowserDownload(RenderBrowserVersionOption option, ManifestBrowserInfo? manifest, CancellationToken cancellationToken)
    {
        var context = new RenderBrowserDownloadContext(
            ...
            ResolveExecutableCandidates(manifest, option),
            ReadJsonFromUrl);
    ...
    private string DownloadBrowserArchive(BrowserDownloadInfo download, ManifestBrowserInfo? manifest, string versionKey, IProgress<RenderBrowserInstallProgress>? progress, CancellationToken cancellationToken)
    {
        ...
        try
        {
            SimpleLogger.Info("开始下载 Render Chromium: " + download.Url);
            ...
            SimpleLogger.Info("Render Chromium 下载完成: " + archivePath);
            return archivePath;
        }
        catch
        {
    ...
    private static void ValidateBrowserArchive(string archivePath, string archiveHash, ManifestBrowserInfo? manifest)
    {
        ...
        if (manifest.Size.HasValue && manifest.Size.Value > 0 && manifest.Size.Value != fileInfo.Length)
            throw new InvalidOperationException("Render Chromium zip 包大小与 manifest 不一致");

        if (!string.IsNullOrWhiteSpace(manifest.Sha256) &&
            !string.Equals(manifest.Sha256, archiveHash, StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException("Render Chromium zip 包 sha256 与 manifest 不一致");
    }
    ...
    private static string? LocateSystemBrowser()
    {
        return EnumerateSystemChromiumCandidates()
    ...
    private static InvalidOperationException CreateBrowserMissingException(string versionKey, string browserDir)
    {
        return new InvalidOperationException(
            "Render Chromium 未安装。请在设置页选择 Chromium 版本后点击下载，或将 Chromium 解压到 " +
            HostConfig.GetRenderBrowserVersionDir(browserDir, versionKey) + " 后重试。");
    }
    ...
    private static string SanitizeFileName(string value)
    {
        ...
        return string.IsNullOrWhiteSpace(result) ? "chromium" : result;
    }
