...
- Printer 启动本地 `easyink-render-host`，用进程内随机 Bearer token 调用 `/v1/render/print-pdf`。
...
- Render Host 固定随 Printer 内置在发布目录 `render\host\easyink-render-host.exe`；Chrome 优先使用内置 `render\browser` 或所选版本的本地缓存。设置页选择 Chrome 版本后可点击下载，普通版本从 Chrome for Testing 官方索引匹配 Windows zip；也可将对应版本 Chrome 手动解压到设置页显示的 Chrome 目录。Windows 7/8.1 自动推荐 Chrome 109，该版本不在 Chrome for Testing 索引中，运行时会自动检测本机已安装 Chrome 109，也支持手动目录或内网 runtime manifest 包。
...
- 若启用 Render，发布目录需要包含 `render\host\easyink-render-host.exe`。Chrome for Testing / headless shell 可随包放入 `render\browser`，也可以在 Printer 设置页按版本下载到本地缓存；离线环境可从 `https://googlechromelabs.github.io/chrome-for-testing/` 下载普通版本。Win7/8.1 使用 Chrome 109 时，可安装本机 Chrome 109、把历史离线包/已安装目录解压到 `%LOCALAPPDATA%\EasyInk.Printer\render\browser\versions\<version>`，或通过 `render\runtime-manifest.json` 指向内网包。Go Render Host 的构建请在 Docker 中执行，避免依赖本机 Go 环境。
