...
| Flutter 插件化示例应用 | [samples/flutter](samples/flutter) |
...
- `renderPdfAsync()` 和 `renderImagesAsync()` 提供 callback 调用入口，适用于不能直接调用 Kotlin `suspend` 函数的宿主。
...
- `samples/flutter` 提供独立 Flutter plugin 与最小宿主，可分别验证 HTML 和 Schema 图片输出。
...
lib/EasyInk.Android/build/repository/com/easyink/easyink-android-render/0.1.0/
...
Flutter sample 通过仓库内生成的 Maven publication 引用 Android SDK，并要求
Flutter 3.38 或更高版本以匹配当前 AAR 的 Kotlin 2.2 metadata。运行时先点击
`Render HTML images`，再点击 `Render Schema images`。
...
```kotlin
val task = EasyInkRenderer.renderPdfAsync(
    context = context,
    request = EasyInkRenderRequest(
        requestId = "receipt-001",
        source = EasyInkRenderSource.Schema(
    ...
    output = File(context.cacheDir, "receipt-001.pdf"),
    callback = object : EasyInkRenderCallback<EasyInkRenderResult> {
        override fun onSuccess(result: EasyInkRenderResult) {
            // result.output 指向生成的 PDF 文件。
        }

        override fun onError(error: EasyInkRenderException) {
            // error.code 可用于区分错误类型。
        }
    },
)
```
...
val task = EasyInkRenderer.renderImagesAsync(
    ...
    callback = object : EasyInkRenderCallback<EasyInkImageRenderResult> {
        override fun onSuccess(result: EasyInkImageRenderResult) {
            // result.files 是按页生成的图片文件列表。
        }
...
`renderPdfAsync()` 和 `renderImagesAsync()` 的 callback 会在 Android 主线程触发。返回的 `EasyInkRenderTask` 可调用 `cancel()`，第一版语义是取消后不再触发 callback；已经开始的底层 WebView 渲染任务会继续完成清理流程。
