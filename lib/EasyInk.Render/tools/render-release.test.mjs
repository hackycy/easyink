...
describe('render release tooling', () => {
  ...
  it('verifyRuntimeBundle validates the embedded viewer files', async () => {
  ...
  it('verifyRuntimeBundle requires bundled runtime metadata', async () => {
    const runtimeRoot = await createRuntimeBundleFixture()
    const manifest = sampleManifest()
  ...
  it('createRuntimeManifest can stamp browser bundle metadata', () => {
    const manifest = createRuntimeManifest({
      ...
      browserBundle: {
        ...
        platform: 'linux-arm64',
        executable: 'chrome',
        archiveName: 'chromium-148.0.7778.97-linux-arm64.tar.gz',
        size: 108,
        sha256: 'b'.repeat(64),
      },
      urlBase: 'https://download.easyink.dev/render',
    ...
    expect(manifest.browser.url).toBe('https://download.easyink.dev/render/browser/148.0.7778.97/linux-arm64/chromium-148.0.7778.97-linux-arm64.tar.gz')
    expect(validateManifest(manifest)).toEqual([])
  })
  ...
  it('buildBrowserBundle packages a local browser directory', async () => {
    ...
    await writeFile(join(browserDir, 'chrome'), 'fake-browser')
    ...
    expect(bundle.executable).toBe('chrome')
...
`)
  ...
  it('createDockerGoBuildArgs cross-compiles a host target in a Go container', () => {
    ...
    expect(args.at(-1)).toContain('./cmd/easyink-render')
  })
  ...
  it('buildBrowserBundle preserves nested browser directories in zip archives', async () => {
    ...
    await writeFile(join(browserDir, 'chrome.exe'), 'fake-browser')
    ...
    const verified = await verifyPackageReference(
      { size: bundle.size, sha256: bundle.sha256, executable: 'chrome.exe' },
      bundle.archivePath,
    )
    expect(verified.errors).toEqual([])
  })
