...
const hostTargets = {
  'win-x64': { goos: 'windows', goarch: 'amd64', executable: 'easyink-render-host.exe', archive: 'tar.gz' },
  'linux-x64': { goos: 'linux', goarch: 'amd64', executable: 'easyink-render-host', archive: 'tar.gz' },
  'linux-arm64': { goos: 'linux', goarch: 'arm64', executable: 'easyink-render-host', archive: 'tar.gz' },
  'darwin-x64': { goos: 'darwin', goarch: 'amd64', executable: 'easyink-render-host', archive: 'tar.gz' },
  'darwin-arm64': { goos: 'darwin', goarch: 'arm64', executable: 'easyink-render-host', archive: 'tar.gz' },
}
...
export async function buildHostPackage(options) {
  ...
  await runCommand(
    'go',
    ['build', '-trimpath', '-ldflags', '-s -w', '-o', executablePath, './cmd/easyink-render-host'],
    {
      cwd: hostRoot,
      env: {
        ...process.env,
  ...
  const archiveName = target.archive === 'zip'
    ? `easyink-render-host-${version}-${platform}.zip`
    : `easyink-render-host-${version}-${platform}.tar.gz`
...
async function main(argv) {
  ...
  if (command === 'build-host') {
    ...
    const hostPackage = await buildHostPackage({ platform, version, outDir })
