...
export async function buildHostPackage(options) {
  ...
  if (options.docker) {
    ...
    await runCommand(
      ...
      [...(options.goCommandArgs ?? []), 'build', '-trimpath', '-ldflags', '-s -w', '-o', executablePath, './cmd/easyink-render'],
      {
        cwd: hostRoot,
        env: {
          ...process.env,
...
export function createDockerGoBuildArgs(options) {
  ...
  const command = [
    'set -e',
    `CGO_ENABLED=0 GOOS=${options.target.goos} GOARCH=${options.target.goarch} /usr/local/go/bin/go build -trimpath -ldflags '-s -w' -o ${quoteSh(`${dockerOutputRoot}/${options.target.executable}`)} ./cmd/easyink-render`,
  ].join('; ')
  return [
    'run',
    '--rm',
...
export async function buildBrowserBundle(options) {
  ...
  const archiveName = platform.startsWith('win-')
    ? `chromium-${version}-${platform}.zip`
    : `chromium-${version}-${platform}.tar.gz`
  const archivePath = resolve(outDir, archiveName)
