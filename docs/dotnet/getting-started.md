...
1. 前往 [EasyInk Releases](https://github.com/hackycy/easyink/releases)，选择 `app-v*` 开头的应用发布版本。
2. 在该版本的 Release assets 中下载 Windows 产物：
   - 推荐安装包：`EasyInkPrinter-Setup-*.exe`
   - 免安装便携包：`EasyInkPrinter-Portable-*.zip`
3. 如果下载的是安装包，运行安装向导；如果下载的是便携包，解压后运行 `EasyInk.Printer.exe`。
