...
连续纸或小票模板应使用 `continuous` 介质和 `stack-flow` 布局策略。

## 元素布局行为

元素的局部布局行为不属于物料私有属性，而是节点级语义：

| 行为 | 字段 | 说明 |
|------|------|------|
| 位置行为 | `placement.mode` | `flow` 跟随前方内容回流；`fixed` 固定在当前位置 |
| 跨页规则 | `break` | 仅在 `auto-sheets` 自动分页中生效 |
| 每页重复 | `repeat.scope` | 分页完成后按每个输出页复制，适合页码、页眉、页脚、水印 |

每页重复不会参与回流和分页计算，因此不会改变页数。设计器中只有源元素可交互，其它页上的重复实例只是输出预览。
