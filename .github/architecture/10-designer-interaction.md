...
```
┌──────────────────────────────────────────────────────────────────────┐
│ Top Bar A: Logo + Materials Entry + Save/Preview                     │
├──────────────────────────────────────────────────────────────────────┤
...
- 品牌和工作台身份
- 物料窗口打开入口（点击按钮切换物料浮动窗口显隐）
- 保存、预览、模板库、更多等全局动作

物料浏览能力由画布内的 `WorkspaceWindow`（kind: `materials`）承载，而不在顶栏内联展示。窗口内部分为两个区域：

- 高频物料区：直线、矩形、圆、文本、图片、二维码、条形码
- 分组物料区：数据、图表、SVG、关系图
