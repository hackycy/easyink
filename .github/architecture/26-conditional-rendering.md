...
- 集合：`any`、`all`、`none`，以及作为值表达式使用的集合数量。
...
type ValueExpression =
  ...
  | {
      kind: 'count'
      value: ConditionPathExpression
    }
```

`count` 只接受数组；非数组值返回 `unknown` 并产生 `CONDITION_COLLECTION_EXPECTED`。对象和数组不能直接参与 `eq/neq/in/notIn`；集合应使用状态操作符、`count` 或量词。
...
```ts
{
  ...
  condition: {
    ...
    operator: 'gt',
    operands: [
      { kind: 'variable', name: 'item', path: 'qty' },
...
嵌套量词使用不同变量名，从而同时引用内外层记录。未定义变量必须在 Schema/编辑器校验阶段暴露。
...
业务若要求“非空且全部满足”，应组合 `isNotEmpty(collection) AND all(collection, condition)`。
...
量词使用相同原则：确定结果优先，否则传播 `unknown`。

- `any`：任一项为 true 即为 true；没有 true 但存在 unknown 时为 unknown；否则为 false。
- `all`：任一项为 false 即为 false；没有 false 但存在 unknown 时为 unknown；否则为 true。
- `none`：等价于 `NOT any`。
...
每访问一个 AST 节点或量词记录消耗一个 step，并允许短路提前结束。超过任一限制时停止该节点求值，返回 `unknown`，产生一次资源限制诊断，再按 `onUnknown` 决定最终状态。首期不允许宿主覆盖这些上限。
...
- Schema validation 递归校验 AST kind、操作符、操作数元数、空逻辑组、量词变量作用域以及深度/节点数限制。
...
- 求值器：全部操作符、显式转换、空值、三值真值表、短路、嵌套量词、step 预算和安全路径。
