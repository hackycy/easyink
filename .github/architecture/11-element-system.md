...
EasyInk 的物料体系覆盖”内容元素”和”结构元素”两类对象。`table / container / chart / svg` 是一级类别。
...
5. **Transaction**：编辑期 mutation 通过 `tx.run(nodeId, draft => {...})`，框架自动产出 `PatchCommand` 并按 `mergeKey` coalesce。
6. **互斥约束**：同一时刻只有一个 `EditingSession` 活动。多选与编辑互斥。容器内子物料编辑 = 退出容器编辑、进入子物料编辑。
