...
- `@easyink/designer-materials-builtin` 和 `@easyink/viewer-materials-builtin` 负责内置物料装配
- 它们消费 `material-*` 包并调用核心注册接口，不把物料依赖重新塞回 `designer` / `viewer`

### `designer` 层

- 负责工作台壳层、画布编辑、窗口系统、状态栏、概览图、历史记录
- 通过插槽、`setupStore` 和 store 暴露宿主扩展面，不把 Viewer、模板选择和导出入口塞进包内
