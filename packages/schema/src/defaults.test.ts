...
import { createDefaultSchema, normalizeDocumentInput, normalizeDocumentSchema } from './defaults'
...
describe('normalizeDocumentSchema', () => {
  ...
  it('rejects unresolved legacy material inputs', () => {
    const input = {
      elements: [{ id: 'legacy', type: 'text', props: { content: 'A' } }],
    }
  ...
  it('accepts canonical material inputs', () => {
    ...
    expect(normalizeDocumentSchema({ elements: [node] }).elements).toEqual([node])
  })
})
