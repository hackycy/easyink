...
- `page.layers` is a page-level render-layer array, not a MaterialNode extension point. Use `page.layers[]` only for whole-page watermarks, and use ordinary elements plus `repeat.scope` for editable headers, footers, logos, page numbers, or data-bound repeated content.
