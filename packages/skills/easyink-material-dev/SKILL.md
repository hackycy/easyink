...
12. Add `propSchemas` only for simple props-bag fields. Use custom `read` and `commit`, `requestPropertyPanel()`, or `SelectionType.getPropertySchema()` when a property lives outside `node.props` or changes multiple fields.
    For text-like props that should be filled from local files, declare `editorOptions.fileImport = { kind: 'text', ... }` and let Designer's `pickFileText` interaction bridge read the file; do not store `File`, local paths, file names, or import state in Schema, and do not implement material-local file inputs.
...
17. Add datasource logic at the right layer. Every material registration declares `binding`: `none`, `ordinary`, `custom`, or `data-contract`. Whole-element scalar binding uses ordinary `BindingRef` in `node.binding` and declares its target prop in `binding.primaryProp`; table-like internal binding owns `datasourceDrop` and cell-level `binding` or `staticBinding` and declares `binding.kind='custom'`; structured data materials declare `binding.kind='data-contract'` with a contract and store `node.binding.kind='data-contract'` mappings from source paths to target model fields.
