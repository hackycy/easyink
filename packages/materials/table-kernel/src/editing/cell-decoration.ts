...
/**
 * @deprecated Use createTableCellDecorationComponent(delegate) instead.
 * Kept for backward compatibility during migration.
 */
export const TableCellDecoration = defineComponent({
  name: 'TableCellDecoration',
  props: {
    rects: { type: Array as () => Rect[], required: true },
    selection: { type: Object as () => Selection, required: true },
  ...
  setup(props) {
    ...
    const colResizeVisible = computed(() => {
      ...
      const cell = t.table.topology.rows[payload.value.row]?.cells[payload.value.col]
      const cs = cell?.colSpan ?? 1
      return payload.value.col + cs - 1 < t.table.topology.columns.length - 1
    })
    ...
    const rowResizeVisible = computed(() => {
      ...
      const cell = t.table.topology.rows[payload.value.row]?.cells[payload.value.col]
      const rs = cell?.rowSpan ?? 1
      return payload.value.row + rs - 1 < t.table.topology.rows.length - 1
    })
    ...
    function onColResizePointerDown(e: PointerEvent) {
      ...
      function onMove(ev: PointerEvent) {
        const delta = ev.clientX - lastX
        lastX = ev.clientX
        props.session.dispatch({ kind: 'command', command: 'resize-column', payload: { index: colIndex, delta, screenDelta: true } })
      }
      function onUp(ev: PointerEvent) {
        el.releasePointerCapture(ev.pointerId)
        el.removeEventListener('pointermove', onMove)
    ...
    function onRowResizePointerDown(e: PointerEvent) {
      ...
      function onMove(ev: PointerEvent) {
        const delta = ev.clientY - lastY
        lastY = ev.clientY
        props.session.dispatch({ kind: 'command', command: 'resize-row', payload: { index: rowIndex, delta, screenDelta: true } })
      }
      function onUp(ev: PointerEvent) {
        el.releasePointerCapture(ev.pointerId)
        el.removeEventListener('pointermove', onMove)
    ...
    return () => {
      ...
      const u = props.unit
      const children = [
        h('div', { style: { position: 'absolute', left: `${r.x}${u}`, top: `${r.y}${u}`, width: `${r.width}${u}`, height: `${r.height}${u}`, border: '2px solid var(--ei-primary, #1890ff)', background: 'rgba(24, 144, 255, 0.08)', pointerEvents: 'none', zIndex: 11, boxSizing: 'border-box' } }),
      ]
      if (colResizeVisible.value) {
        children.push(h('div', { style: { position: 'absolute', left: `${r.x + r.width}${u}`, top: `${r.y}${u}`, width: '6px', height: `${r.height}${u}`, marginLeft: '-3px', cursor: 'col-resize', pointerEvents: 'auto', zIndex: 12, background: resizeMeta.value?.handle === 'col' ? 'rgba(24, 144, 255, 0.15)' : undefined }, onPointerdown: onColResizePointerDown }))
      }
      if (rowResizeVisible.value) {
        children.push(h('div', { style: { position: 'absolute', left: `${r.x}${u}`, top: `${r.y + r.height}${u}`, width: `${r.width}${u}`, height: '6px', marginTop: '-3px', cursor: 'row-resize', pointerEvents: 'auto', zIndex: 12, background: resizeMeta.value?.handle === 'row' ? 'rgba(24, 144, 255, 0.15)' : undefined }, onPointerdown: onRowResizePointerDown }))
      }
      return children
    }
  },
