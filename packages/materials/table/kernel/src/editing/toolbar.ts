...
import { tableModel, tableProjection } from './canonical'
...
export function createTableToolbarGroups(
  ...
  node: MaterialNode<unknown> | undefined,
  ...
  const cellRole = kind === 'data'
    ? projection.topology.rows[selection.row]?.role ?? null
    : null
  ...
  if (!isDataArea) {
    const rowId = projection.rowIds[selection.row]
    ...
    groups.push({
      ...
      actions: [
        ...
        { id: 'insert-row-below', label: t('materials.table.action.insertRowBelow'), icon: IconTableInsertRowBelow, command: 'insert-row-below' },
        {
          ...
          command: 'remove-row',
          disabled: !band || band.rows.length <= 1,
          danger: true,
        },
      ],
    })
  ...
  groups.push({
    ...
    actions: [
      ...
      { id: 'insert-col-right', label: t('materials.table.action.insertColRight'), icon: IconTableInsertColRight, command: 'insert-col-right' },
      {
        ...
        command: 'remove-col',
        disabled: projection.topology.columns.length <= 1,
        danger: true,
      },
    ],
  })
  ...
  if (!isDataArea) {
    const cell = projection.topology.rows[selection.row]?.cells[selection.col]
