...
import { isEditableTableNode, tableProjection } from './canonical'
...
export function computePlaceholderHeight(
  ...
  const repeatRow = topology.rows.find(r => r.role === 'repeat-template')
  if (!repeatRow)
    return 0
...
function computeRowHeightsWithPlaceholders(node: MaterialNode<unknown>, placeholderCount: number, hidden?: readonly boolean[]): number[] {
  const topology = tableProjection(node).topology
  const repeatRow = topology.rows.find(r => r.role === 'repeat-template')
  const rowScale = computeRowScaleWithVirtualRows(
    topology.rows,
  ...
  return topology.rows.map((row, index) => hidden?.[index] ? 0 : row.height * rowScale)
}
...
function hitTestGridCellWithRowHeights(
  ...
  const colWidths = computeColumnWidths(tableProjection(node).topology.columns, node.width)
...
export function computeCellRectWithPlaceholders(
  ...
  const topology = tableProjection(node).topology
  const rect = computeCellRect(topology, node.width, node.height, row, col, hidden)
  if (!rect)
    return null
  ...
  const repeatIdx = topology.rows.findIndex(r => r.role === 'repeat-template')
...
function computeCellRectWithRowHeights(
  ...
  const { columns, rows } = tableProjection(node).topology
...
export function hitTestWithPlaceholders(
  ...
  if (placeholderCount <= 0)
    return hitTestGridCell(tableProjection(node).topology, node.width, node.height, relX, relY, hidden)

  const topology = tableProjection(node).topology
  const repeatIdx = topology.rows.findIndex(r => r.role === 'repeat-template')
  if (repeatIdx < 0)
    return hitTestGridCell(topology, node.width, node.height, relX, relY, hidden)
...
export function createTableGeometry(delegate: TableEditingDelegate): MaterialGeometry {
  const getHidden = (node: MaterialNode<unknown>) => delegate.getHiddenRowMask?.(node)
  ...
  return {
    ...
    hitTest(point, node: MaterialNode) {
      ...
      const owner = resolveMergeOwner(tableProjection(node).topology, gridCell.row, gridCell.col)
      return {
        type: 'table.cell',
        nodeId: node.id,
