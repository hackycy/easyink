...
export function createChartLineExtension(_context: MaterialExtensionContext): MaterialDesignerExtension {
  return {
    renderContent(nodeSignal, container) {
      ...
      const mount = mountECharts(chartEl, createChartLinePreviewOption(getNodeModel<ChartLineProps>(nodeSignal.get())))
      const unsubscribe = nodeSignal.subscribe((node) => {
        mount.update(createChartLinePreviewOption(getNodeModel<ChartLineProps>(node)))
      })
