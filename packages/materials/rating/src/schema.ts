...
export function createRatingNode(partial?: Partial<MaterialNode>, unit?: string): MaterialNode {
  ...
  const partialModel = (partial?.model ?? {}) as Partial<RatingProps>

  if (partialNode)
    delete partialNode.model

  return canonicalizeMaterialNode(RATING_TYPE, {
    id: generateId('rate'),
    type: RATING_TYPE,
    x: 0,
    y: 0,
    ...
    model: {
      ...
      ...partialModel,
      character: normalizeRatingCharacter(partialModel.character ?? RATING_DEFAULTS.character),
    },
    ...partialNode,
  })
}
