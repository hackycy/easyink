 ...
 * - editing 级（双击进入）：创建 contenteditable 富文本编辑器
 * - 绑定模式下阻止进入 editing 级
 */
export const richTextInteractionStrategy: InteractionStrategy = {
  ...
  onEnterEditing(material, context) {
    ...
    function commitEdit(): void {
      ...
      const newContent = materialEl!.innerHTML ?? ''
    ...
    function onKeyDown(e: KeyboardEvent): void {
      if (e.key === 'Escape') {
        // 取消编辑，恢复原始内容
        materialEl!.innerHTML = oldContent
        materialEl!.contentEditable = 'false'
        materialEl!.style.cursor = ''
        cleanup()
      }
