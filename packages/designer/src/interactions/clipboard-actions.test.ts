...
describe('createClipboardActions', () => {
  ...
  it('normalizes ancestor and descendant roots for copy, paste, and duplicate', () => {
    const profile = createTestCompiledMaterialProfile()
    const child = profile.createNode('box', { id: 'child' })
    const owner = profile.createNode('container', { id: 'owner', slots: { content: [child] } })
    const store = makeStore([owner], ['owner', 'child'], undefined, profile)
    const selected = () => [store.getElementById('owner')!, store.getElementById('child')!]
    const actions = createClipboardActions(store, selected, profile)
    actions.copySelection()
    expect(store.clipboard).toHaveLength(1)
    expect(store.clipboard[0]?.slots.content).toHaveLength(1)
    actions.pasteClipboard()
    expect(store.schema.elements).toHaveLength(2)
    expect(store.documentTransactions.historyEntries).toHaveLength(1)
    store.documentTransactions.undo()
